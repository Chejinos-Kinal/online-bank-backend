...
export const getUser = async(req, res) => {
    try {
        ...
        let existingUser = await User.findOne({_id: userId, status: true}).populate('idAccount', ['balance'])
...
// ----------------------------------------------- USER -----------------------------------------------
 export const updateUserClient = async(req, res) => {
    try {
        ...
        if (userIdL.toString() !== userIdU.toString()) return res.status(401).send({ message: 'You only can update your user' });
        const keys = Object.keys(data);
        const disallowedFields = keys.filter(key => !allowedFields.includes(key));
        if (disallowedFields.length > 0) return res.status(400).send({ message: `You cannot update the following fields: ${disallowedFields.join(', ')}` });
        if (keys.length === 0) return res.status(400).send({ message: 'No fields to update' });
 ...
 export const getUserClient = async (req, res) => {
    try {
        ...
        if (userIdL.toString()!== userId.toString()) return res.status(401).send({ message: 'You only can get your user'})
        let existingUser = await User.findOne({_id: userId, status: true}).populate('idAccount', ['balance'])
        if(!existingUser) return res.status(404).send({ message: 'User not found' })
        return res.send(existingUser)
    } catch (error) {
